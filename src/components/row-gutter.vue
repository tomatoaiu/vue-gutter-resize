<template>
  <section ref="gutter" :style="`width: ${width}px; height: ${height}px;`">
      <div
        :style="`height: calc(${rowArray[0]}% - ${gutterSize}px);`">
        <div
          class="pane pane-h" :style="`height: calc(${100}% - ${gutterSize}px);`">
      ...
      <div
        ...
        :style="`height: calc(${rowArray[n]}% - ${gutterSize}px);`">
        <div
          class="gutter gutter-h" draggable="true"
          :style="`height: ${gutterSize}px; width: ${width}px; background-color: ${color || colors[n - 1]};`"
          @drag="e => { drag(e, n - 1) }">
        </div>
        <div
          class="pane pane-h" :style="`height: calc(${100}% - ${gutterSize}px);`">
...
export default {
  name: 'rowGutter',
  mixins: [ gutter ],
  props: ['width', 'height', 'gutterSize', 'color', 'row', 'colors'],
  data () {
    return {
      rowArray: []
    }
